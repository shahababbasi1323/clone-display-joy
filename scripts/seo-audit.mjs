...
function main() {
  let files = walk(DIST);
  // Admin SPA shells are regenerated by build-admin-shells.mjs as clean noindex
  // pages - they don't need the audit/fix pass and we don't want their meta
  // overwritten with homepage content.
  files = files.filter(f => !f.includes("/wah/") && !f.endsWith("/wah/index.html"));
